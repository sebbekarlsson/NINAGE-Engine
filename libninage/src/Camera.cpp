...
void Camera::draw(float delta) {
    ...
    game->drawText("FPS: " + std::to_string(game->getFPS()), "/usr/local/opt/ninage/assets/font/bits.ttf", 16, col);
    glPopMatrix();

    glPushMatrix();
    glTranslatef(16.0f, 32.0f + 8.0f, 0.0f);
    game->drawText("DELTA: " + std::to_string(delta), "/usr/local/opt/ninage/assets/font/bits.ttf", 16, col);
    glPopMatrix();
